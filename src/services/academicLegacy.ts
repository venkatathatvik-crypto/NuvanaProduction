...
export const getStudentTests = async (
  ...
  const testsWithDetails = await Promise.all(
    testsData.map(async (test: any) => {
      ...
      const examTypeCategory = Array.isArray(test.exam_types)
        ? test.exam_types[0]?.type
        : test.exam_types?.type;
      ...
      return {
        ...
        examTypeId: test.exam_type_id,
        examTypeName,
        examTypeCategory: examTypeCategory as "Internal Assessment" | "School Exam" | undefined,
        createdAt: test.created_at,
        dueDate: test.due_date || undefined,
        questionCount,
        totalMarks,
