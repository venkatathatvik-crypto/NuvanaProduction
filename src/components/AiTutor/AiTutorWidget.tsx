...
import { Brain, X, PenTool, Maximize2, Move } from "lucide-react";
...
const AiTutorWidget = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      // Check if click is within a Radix UI portal (e.g., Select dropdown)
      const isInPortal = (target as Element).closest?.("[data-radix-portal]");

      // Check if click is on any Select-related element
      const isSelectDropdown = !!(target as Element).closest?.(
        '[role="listbox"], [role="option"], .select-content, [data-select-trigger], [data-select-value]'
      );
      ...
      if (
        ...
        !isInPortal && // Don't close if clicking in a portal
        !isSelectDropdown // Don't close if clicking on select dropdown
      ) {
        setIsOpen(false);
      }
    };
  ...
  useEffect(() => {
    ...
    if (
      ...
      pathname.includes("/student/tests/take")
    ) {
      setIsVisible(false);
      setIsOpen(false);
      return;
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <div
              className="flex items-center justify-between p-2 bg-secondary/30 border-b border-border/50 cursor-move"
      ...
      >
        ...
        >
          {(() => {
            ...
            if (isTeacher) return <PenTool className="w-7 h-7" />;
